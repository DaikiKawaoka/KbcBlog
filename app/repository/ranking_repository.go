...
func KBCArticleRankingTop10(rankingType int, period string) ([]*model.RankingUser, error) {
	...
	if rankingType == 0 {
		if period == "all"{
			...
			// 1ヶ月以内
			query = `SELECT DISTINCT * FROM((SELECT u.id id, u.name name, u.comment comment, u.imgdata64 imgdata64, u.sex sex, COUNT(al.id) count
			FROM users u LEFT JOIN articles a on a.userid = u.id
			LEFT join article_likes al on al.articleid = a.id
			WHERE u.id <> 920437694 and al.created >= DATE_ADD(NOW(), interval -1 month)
			GROUP BY u.id,u.name)
			UNION
			(SELECT u.id id, u.name name,u.comment comment, u.imgdata64 imgdata64, u.sex sex, COUNT(al.id) count
			FROM users u LEFT JOIN articles a on a.userid = u.id
			LEFT join article_likes al on al.articleid = a.id
			WHERE u.id <> 920437694
			GROUP BY u.id,u.name
		...
		if period == "all"{
			...
			// 1ヶ月以内
			query = `SELECT DISTINCT * FROM((SELECT u.id id, u.name name,u.comment comment, u.imgdata64 imgdata64, u.sex sex, COUNT(a.id) count
			FROM users u LEFT JOIN articles a on a.userid = u.id
			WHERE u.id <> 920437694 and a.created >= DATE_ADD(NOW(), interval -1 month)
			GROUP BY u.id,u.name,u.comment)
			UNION
			(SELECT u.id id, u.name name,u.comment comment, u.imgdata64 imgdata64, u.sex sex, COUNT(a.id) count
			FROM users u LEFT JOIN articles a on a.userid = u.id
			WHERE u.id <> 920437694
			GROUP BY u.id,u.name,u.comment
			HAVING count = 0)) AS new ORDER BY count desc LIMIT 10;`
...
func KBCQuestionRankingTop10(rankingType int, period string) ([]*model.RankingUser, error) {
	...
	if rankingType == 0 {
		...
		if period == "all"{
			...
			query = `SELECT DISTINCT * FROM((SELECT u.id id, u.name name, u.comment comment, u.imgdata64 imgdata64, u.sex sex, COUNT(q.id) count
			FROM users u LEFT JOIN questions q on q.userid = u.id
			WHERE u.id <> 920437694 and q.created >= DATE_ADD(NOW(), interval -1 month)
			GROUP BY u.id,u.name)
			UNION
			(SELECT u.id id, u.name name,u.comment comment, u.imgdata64 imgdata64, u.sex sex, COUNT(q.id) count
			FROM users u LEFT JOIN questions q on q.userid = u.id
			WHERE u.id <> 920437694
			GROUP BY u.id,u.name
			HAVING count = 0)) AS new ORDER BY count desc LIMIT 10;`
